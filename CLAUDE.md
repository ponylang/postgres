...
- `ponylang/lori` 0.13.1 (TCP networking, STARTTLS support)
...
`_SessionSCRAMAuthenticating` handles the multi-step SCRAM-SHA-256 exchange after `_SessionConnected` receives an AuthSASL challenge. It mixes in `_ConnectedState` (for `on_received`/TCP write) and `_NotAuthenticated`. Fields store the client nonce, client-first-bare, password, expected server signature, and `_server_verified` — a boolean set only after successful `AuthenticationSASLFinal` verification and checked by `on_authentication_ok` before transitioning to `_SessionLoggedIn`. `on_authentication_sasl_final` only accepts the `v=<verifier>` form of server-final-message. RFC 5802 §7 also defines `e=<server-error>`, but PostgreSQL never embeds SCRAM errors in SASLFinal — it conveys them via `ErrorResponse`, which is handled separately by `on_error_response`. Any SASLFinal content not starting with `v=` is treated as a protocol violation.
...
- `PreparedQuery` — val class wrapping a query string + `Array[FieldDataTypes] val` params (extended query protocol, single statement only). Typed params (`I16`, `I32`, `I64`, `F32`, `F64`, `Bool`, `Array[U8] val`, `PgArray`, `PgComposite`, `PgTimestamp`, `PgTime`, `PgDate`, `PgInterval`) use binary wire format with explicit OIDs; `String` and `None` use text format with server-inferred types
...
- `_FieldDataEq` — package-private primitive with shared equality logic for `FieldData` values. Used by `Field.eq()`, `PgArray.eq()`, and `PgComposite.eq()`. Methods: `apply(a, b)`, `nullable(a, b)`
...
- `FieldDataTypes` = `(Array[U8] val | Bool | F32 | F64 | I16 | I32 | I64 | None | PgArray | PgComposite | PgDate | PgInterval | PgTime | PgTimestamp | String)` — closed union for encode path (query parameters)
...
- `PgComposite` — val class wrapping `type_oid: U32`, `field_oids: Array[U32] val`, `field_names: Array[String] val`, and `fields: Array[(FieldData | None)] val`. Two constructors: `create(type_oid, field_oids, field_names, fields)?` validates parallel arrays have equal size (used by decode path), `from_fields(type_oid, descriptors)` takes `Array[(String, U32, (FieldData | None))] val` triples and splits internally (preferred for user construction). Methods: `size`, `apply(i) ?`, `field(name) ?` (first-match on duplicate names), `eq`, `field_data_eq`, `string` (PostgreSQL composite literal format). Implements `FieldData & FieldDataEquatable & Equatable[PgComposite]`
...
- `SessionStatusNotify` interface (tag) — lifecycle callbacks (connected, connection_failed(reason), authenticated, transaction_status, notification, notice, parameter_status, shutdown). `pg_session_connection_failed` fires for any pre-ready failure: transport-level, TLS, unsupported authentication method, or server rejection during startup
...
- `ConnectionFailureReason` = `(ConnectionFailedDNS | ConnectionFailedTCP | ConnectionFailedTimeout | ConnectionFailedTimerError | SSLServerRefused | TLSHandshakeFailed | TLSAuthFailed | UnsupportedAuthenticationMethod | ServerVerificationFailed | InvalidPassword | InvalidAuthorizationSpecification | TooManyConnections | InvalidDatabaseName | ServerRejected)`. Delivered via `pg_session_connection_failed` callback. The first nine variants are primitives (transport and client-side detections). The last five are `class val` wrappers around `ErrorResponseMessage` (accessed via `response()`) mapped from the server's SQLSTATE code: `InvalidPassword` (28P01), `InvalidAuthorizationSpecification` (28000), `TooManyConnections` (53300), `InvalidDatabaseName` (3D000), and `ServerRejected` (fallback for any other server ErrorResponse during startup). `_ConnectionFailureReasonFromError` (package-private primitive) centralizes the SQLSTATE → variant mapping
...
- `_ArrayEncoder` (`_array_encoder.pony`): encodes `PgArray` to binary array wire format. Dispatches element encoding on Pony runtime types. String elements routed by `element_oid`: uuid → `_UuidBinaryCodec`, jsonb → `_JsonbBinaryCodec`, oid → `_OidBinaryCodec`, numeric → `_NumericBinaryCodec`; all others → raw UTF-8 bytes. Coupling: element encoding must stay in sync with `_CompositeEncoder`, `_FrontendMessage.bind()`, and `_binary_codecs.pony`
- `_CompositeEncoder` (`_composite_encoder.pony`): encodes `PgComposite` to binary composite wire format. Per-field encoding delegates to type-specific codecs, with recursive support for nested composites and array-typed fields. Coupling: element encoding must stay in sync with `_ArrayEncoder`, `_FrontendMessage.bind()`, and `_binary_codecs.pony`
- `CodecRegistry` (`codec_registry.pony`): maps OIDs to codecs. Adds `_custom_array_element_oids: Map[U32, U32] val` field for custom array type registrations. Adds `_composite_field_descriptors: Map[U32, (Array[String] val, Array[U32] val)] val` field for composite type registrations. Default constructor populates all built-ins. `with_codec(oid, codec)?` returns a new registry with the codec registered (errors if OID already registered). `with_enum_type(oid)?` registers text-passthrough codecs for both text and binary formats atomically (errors if OID already registered in either codec map or collides with an array OID). `with_composite_type(oid, field_descriptors)?` registers a user-defined composite type mapping (partial — rejects already-registered OIDs, array OID collisions, empty descriptors, self-referential field OIDs). Field descriptors are `Array[(String, U32)] val` of (name, oid) pairs. `with_array_type(array_oid, element_oid)?` returns a new registry with the custom array mapping added (errors if element_oid is an array OID, array_oid collides with a registered OID, array_oid is already a custom array OID, array_oid is already a custom element OID, or array_oid == element_oid). `array_oid_for(element_oid): U32` returns the array OID for the given element OID. `decode()` intercepts array OIDs before normal codec dispatch via two-phase array decoding: `_parse_binary_array`/`_parse_text_array` extract structure and raw element bytes (errors fall back), then `_decode_array_elements` decodes each element via the element codec (errors propagate). After array checks, intercepts composite OIDs before scalar codec dispatch. `has_binary_codec()` checks array OIDs and composite OIDs too. `_with_codec` constructor (type-private) used internally by `with_codec`; `_with_enum_type` constructor used internally by `with_enum_type`; `_with_composite_type` constructor used internally by `with_composite_type`
- `_ParamEncoder` (`_param_encoder.pony`): derives PostgreSQL OIDs from `FieldDataTypes` parameter values for Parse messages. Takes `registry: CodecRegistry` parameter; `PgArray` arm uses `registry.array_oid_for(a.element_oid)`; `PgComposite` arm returns `c.type_oid`

**Encode error handling:** `_FrontendMessage.bind()` is partial — it errors if parameter encoding fails. Takes `registry: CodecRegistry` parameter (with default `CodecRegistry`); pre-encodes `PgArray` parameters via `_ArrayEncoder` and `PgComposite` parameters via `_CompositeEncoder` before the `recover val` block. `_QueryReady.try_run_query()` uses a build-before-transition pattern: wire messages are constructed before transitioning to an in-flight state, so encode errors deliver `DataError` to the receiver without leaving the state machine inconsistent. Pipeline queries build message parts into an `iso` array in `ref` scope (where error handling has full access to the session and receiver), then consume the array into a `recover val` block for concatenation.
...
**Ports**: Mock server tests use ports in the 7669–7730 range and 9667–9668. **Port 7680 is reserved by Windows** (Update Delivery Optimization) and will fail to bind on WSL2 — do not use it. Composite integration tests don't use mock servers (they use the real PostgreSQL containers).
