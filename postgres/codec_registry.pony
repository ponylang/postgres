...
class val CodecRegistry
  ...
  new val _with_composite_type(
    base: CodecRegistry,
    oid: U32,
    names: Array[String] val,
    oids: Array[U32] val)
  =>
    """
  ...
  =>
    ...
    if _ArrayOidMap.is_array_oid(oid) then
      ...
      return _decode_array_elements(
        parsed._1, format, parsed._2, depth)?
    end
    ...
    // Check composite OIDs — no fallback; parsing errors propagate
    // because composites are explicitly registered (unlike arrays,
    // which might be unrecognized formats). Field count mismatches
    // indicate schema drift.
    if _composite_field_descriptors.contains(oid) then
      let parsed =
        if format == 0 then _parse_text_composite(oid, data)?
        else _parse_binary_composite(data)? end
      return _decode_composite_fields(
        oid, format, parsed, depth)?
    end

    if format == 0 then
      let codec =
        try _text_codecs(oid)?
        else return String.from_array(data)
        end
      codec.decode(data)?
    else
      let codec =
        try _binary_codecs(oid)?
        else return RawBytes(data)
        end
      codec.decode(data)?
    end
  ...
  =>
    ...
    let raw_elements =
      recover val
        let elems =
          Array[(Array[U8] val | None)](dim_size.usize())
        var offset: USize = 20
        var i: I32 = 0
        while i < dim_size do
  ...
  =>
    ...
    let raw_elements: Array[(Array[U8] val | None)] val =
      recover val
        ...
        while pos < end_pos do
          if s(pos)? == '"' then
            // Quoted element — use iso array for byte
            // accumulation
            pos = pos + 1
  ...
  fun _decode_array_elements(
    element_oid: U32,
    format: U16,
    raw_elements: Array[(Array[U8] val | None)] val,
    depth: USize)
    : PgArray ?
  =>
    """
    ...
    decoding. The text path is not affected (it looks up `element_oid` from
    the registry), but the guard applies uniformly.
    """
    if _ArrayOidMap.is_array_oid(element_oid) then error end
    if _custom_array_element_oids.contains(element_oid) then
      error
    end
    let elements =
      recover iso
        let elems =
          Array[(FieldData | None)](raw_elements.size())
  ...
  =>
    ...
    let raw_fields: Array[(U32, (Array[U8] val | None))] val =
      recover val
        let fields =
          Array[(U32, (Array[U8] val | None))](
            field_oids.size())
        var pos: USize = 1  // skip opening '('
        let end_pos = s.size() - 1  // before closing ')'
        ...
        do
          ...
          if pos == end_pos then
            if after_comma then
              // Trailing NULL — comma preceded closing paren
              fields.push((field_oid, None))
          ...
          elseif s(pos)? == ',' then
            ...
            field_idx = field_idx + 1
          elseif s(pos)? == '"' then
            // Quoted field — doubled-character escaping
            pos = pos + 1
            var buf = recover iso Array[U8] end
            while pos < end_pos do
              let ch = s(pos)?
        ...
        if field_idx != field_oids.size() then
          error
        end
        // Verify all input consumed (extra fields = schema mismatch)
        if pos != end_pos then error end
        fields
      end
    raw_fields

  fun _decode_composite_fields(
    oid: U32,
    format: U16,
    parsed: Array[(U32, (Array[U8] val | None))] val,
    depth: USize)
    : PgComposite ?
  =>
    """
    ...
    """
    (let names, let registered_oids) =
      _composite_field_descriptors(oid)?

    if parsed.size() != registered_oids.size() then
      error
    end

    let wire_oids: Array[U32] val =
      recover val
    ...
    let fields =
      recover iso
        let f =
          Array[(FieldData | None)](parsed.size())
    ...
    PgComposite(
      oid, wire_oids, names, consume fields)?
