class val PreparedQuery
  ...
  `PgTimestamp`, `PgTime`, `PgDate`, `PgInterval`). `String` values are
  sent in text format with OID 0 (server infers the type), preserving
  backward compatibility — the server parses the text representation for
  whatever type the column expects. Use `None` for NULL.
