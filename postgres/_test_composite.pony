...
primitive \nodoc\ _TestCompositeBinaryBuilder
  ...
  """
  fun apply(
    fields: Array[(U32, (Array[U8] val | None))] val)
    : Array[U8] val
  =>
    try
...
class \nodoc\ iso _TestCompositeFromFieldsEmpty
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) =>
    let c =
      PgComposite.from_fields(
        16400,
        recover val
          Array[(String, U32, (FieldData | None))]
        end)
    h.assert_eq[USize](0, c.size())

class \nodoc\ iso _TestCompositeCreateValidation
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso _TestCompositeNamedAccessMiss
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso _TestCompositeFieldDataEq
  is UnitTest
  fun name(): String =>
...
// ============================================================
class \nodoc\ iso _TestCompositeRegistration
  is UnitTest
  fun name(): String =>
    "CodecRegistry/WithCompositeType"

  fun apply(h: TestHelper) ? =>
    let fields: Array[(String, U32)] val =
      recover val
        [ as (String, U32):
          ("street", 25)
          ("city", 25)
...
class \nodoc\ iso
  _TestCompositeRegistrationRejectsEmpty
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestCompositeRegistrationRejectsExistingOid
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestCompositeRegistrationRejectsEnumOid
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) =>
    ...
    h.assert_error({()? =>
      let r =
        CodecRegistry.with_enum_type(50000)?
      r.with_composite_type(50000, fields)?
    })

class \nodoc\ iso
  _TestCompositeRegistrationRejectsArrayOid
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) =>
    ...
    h.assert_error({()? =>
      let r =
        CodecRegistry.with_array_type(50000, 25)?
      r.with_composite_type(50000, fields)?
    })

class \nodoc\ iso
  _TestCompositeRegistrationRejectsSelfRef
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestCompositeRegistrationRejectsDuplicate
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestCompositeRegistrationBlocksWithCodec
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) =>
    ...
    // with_codec should reject an OID already
    // registered as composite
    h.assert_error({()? =>
      let r =
        CodecRegistry.with_composite_type(
          16400, fields)?
      r.with_codec(
        16400, _TextPassthroughBinaryCodec)?
    })

class \nodoc\ iso
  _TestCompositeRegistrationBlocksWithEnumType
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) =>
    ...
    // with_enum_type should reject an OID already
    // registered as composite
    h.assert_error({()? =>
      let r =
        CodecRegistry.with_composite_type(
...
class \nodoc\ iso
  _TestCompositeRegistrationBlocksWithArrayType
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) =>
    ...
    // with_array_type should reject using a composite
    // OID as array_oid
    h.assert_error({()? =>
      let r =
        CodecRegistry.with_composite_type(
...
class \nodoc\ iso _TestCompositeEqSizeMismatch
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso _TestCompositeEqWithNulls
  is UnitTest
  fun name(): String =>
...
// Binary decode tests
// ============================================================
class \nodoc\ iso _TestBinaryDecodeCompositeSimple
  is UnitTest
  fun name(): String =>
    "Codec/Binary/Composite/Simple"

  fun apply(h: TestHelper) ? =>
    let fields: Array[(U32, (Array[U8] val | None))] val =
      recover val
        [ as (U32, (Array[U8] val | None)):
          (25,
            _TestCompositeBinaryBuilder.text_bytes(
              "hello"))
          (23,
            _TestCompositeBinaryBuilder.int4_bytes(
              42))]
      end
...
class \nodoc\ iso
  _TestBinaryDecodeCompositeWithNull
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestBinaryDecodeCompositeNested
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    let inner_data =
      _TestCompositeBinaryBuilder(inner_fields)
    ...
    let outer_data =
      _TestCompositeBinaryBuilder(outer_fields)

    let inner_desc: Array[(String, U32)] val =
      recover val
        [ as (String, U32): ("a", 25); ("b", 23)]
      end
    let outer_desc: Array[(String, U32)] val =
      recover val
        [ as (String, U32):
          ("label", 25); ("nested", 16400)]
      end
    let registry =
      CodecRegistry
    ...
    let result =
      registry.decode(16500, 1, outer_data)?
    match result
    | let c: PgComposite =>
      match c(0)?
      | let s: String =>
        h.assert_eq[String]("outer", s)
      else h.fail("Expected String")
      end
      match c(1)?
...
class \nodoc\ iso
  _TestBinaryDecodeCompositeFieldCountMismatch
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) =>
    ...
    h.assert_error({()? =>
      ...
      let data =
        _TestCompositeBinaryBuilder(fields)
      let descriptors: Array[(String, U32)] val =
        recover val
          [ as (String, U32):
            ("name", 25); ("age", 23)]
...
class \nodoc\ iso
  _TestBinaryDecodeCompositeDepthGuard
  is UnitTest
  fun name(): String =>
    "Codec/Binary/Composite/DepthGuard"

  fun apply(h: TestHelper) =>
    // Build deeply nested composite data that exceeds
    // the depth limit. We register 18 composite types,
    // each nesting the previous one. At decode time,
    // depth should exceed the limit of 16.
    h.assert_error({()? =>
      var registry = CodecRegistry
      // Register composites: OIDs 60000..60017
      // 60000: (text)
      let leaf_desc: Array[(String, U32)] val =
        recover val
          [ as (String, U32): ("val", 25)]
        end
      registry =
        registry.with_composite_type(
          60000, leaf_desc)?
      var oid: U32 = 60001
      while oid <= 60017 do
        let inner_oid = oid - 1
...
// Text decode tests
// ============================================================
class \nodoc\ iso _TestTextDecodeCompositeSimple
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestTextDecodeCompositeWithNull
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    | let c: PgComposite =>
      ...
      | None => None // expected
        ...
        h.fail(
          "Expected None for trailing empty field")
      end
    else
      h.fail("Expected PgComposite")
    end
...
class \nodoc\ iso
  _TestTextDecodeCompositeEmptyString
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso _TestTextDecodeCompositeQuoted
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    | let c: PgComposite =>
      match c(0)?
      | let s: String =>
        h.assert_eq[String]("has,comma", s)
      else h.fail("Expected String a")
      end
      match c(1)?
      | let s: String =>
        h.assert_eq[String]("has\"quote", s)
...
class \nodoc\ iso _TestTextDecodeCompositeNested
  is UnitTest
  fun name(): String =>
    "Codec/Text/Composite/Nested"

  fun apply(h: TestHelper) ? =>
    // Nested composite in text format: the inner
    // composite appears as a quoted string with
    // doubled-double-quote escaping for inner quotes.
    // PostgreSQL renders:
    // ("(inner_val,99)",outer_label)
    let inner_desc: Array[(String, U32)] val =
      recover val
        [ as (String, U32):
          ("a", 25); ("b", 23)]
    ...
    // Outer composite text: nested field is quoted,
    // inner parens preserved
    let data: Array[U8] val =
      "(\"(inner_val,99)\",outer_label)".array()
    ...
    | let c: PgComposite =>
      ...
      | let inner: PgComposite =>
        match inner(0)?
        | let s: String =>
          h.assert_eq[String]("inner_val", s)
        else h.fail("Expected inner String a")
        end
        // Text codec for int4 (OID 23) decodes
        // to I32
        match inner(1)?
        | let v: I32 => h.assert_eq[I32](99, v)
      ...
      match c(1)?
      | let s: String =>
        h.assert_eq[String]("outer_label", s)
...
class \nodoc\ iso
  _TestTextDecodeCompositeBackslash
  is UnitTest
  fun name(): String =>
    "Codec/Text/Composite/Backslash"

  fun apply(h: TestHelper) ? =>
    // PostgreSQL doubles backslashes in composite
    // text output: C:\Users\file ->
    // ("C:\\Users\\file")
    let data: Array[U8] val =
      "(\"C:\\\\Users\\\\file\")".array()
    let descriptors: Array[(String, U32)] val =
...
class \nodoc\ iso
  _TestTextDecodeCompositeFieldCountMismatch
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestTextDecodeCompositeNullFirst
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    | let c: PgComposite =>
      ...
      | None => None // expected
        ...
        h.fail("Expected None for first field")
      end
      match c(1)?
      | let s: String =>
        h.assert_eq[String]("hello", s)
...
class \nodoc\ iso
  _TestTextDecodeCompositeFewerFields
  is UnitTest
  fun name(): String =>
    "Codec/Text/Composite/FewerFields"

  fun apply(h: TestHelper) =>
    // 1 field in text, 2 registered; should error
    h.assert_error({()? =>
      let data: Array[U8] val =
        "(hello)".array()
      let descriptors: Array[(String, U32)] val =
...
// Encode tests
// ============================================================
class \nodoc\ iso _TestCompositeEncoderSimple
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    let encoded =
      _CompositeEncoder(c, registry)?
    let decoded =
      registry.decode(16400, 1, encoded)?
    match decoded
    | let d: PgComposite =>
...
class \nodoc\ iso _TestCompositeEncoderWithNull
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso _TestCompositeEncoderNested
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    let outer =
      PgComposite(
        ...
        recover val
          [ as String: "label"; "inner"]
        end,
        recover val
          [ as (FieldData | None): "top"; inner]
        end)?

    let encoded =
      _CompositeEncoder(outer, registry)?
    let decoded =
      registry.decode(16500, 1, encoded)?
    match decoded
    | let d: PgComposite =>
      match d(0)?
      | let s: String =>
        h.assert_eq[String]("top", s)
      else h.fail("Expected String")
      end
      match d(1)?
      | let inner_d: PgComposite =>
        match inner_d(0)?
        | let s: String =>
          h.assert_eq[String]("nested_val", s)
        else h.fail("Expected inner String")
        end
      else
        h.fail("Expected nested PgComposite")
...
class \nodoc\ iso
  _TestCompositeEncoderI16Roundtrip
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestCompositeEncoderI64Roundtrip
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestCompositeEncoderF32Roundtrip
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestCompositeEncoderF64Roundtrip
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    | let d: PgComposite =>
      match d(0)?
      | let v: F64 =>
        h.assert_eq[F64](F64(2.718281828), v)
...
class \nodoc\ iso
  _TestCompositeEncoderBoolRoundtrip
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestCompositeEncoderByteaRoundtrip
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    | let d: PgComposite =>
      match d(0)?
      | let v: Bytea =>
        h.assert_eq[Bytea](Bytea(data), v)
...
class \nodoc\ iso
  _TestCompositeEncoderDateRoundtrip
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    | let d: PgComposite =>
      match d(0)?
      | let v: PgDate =>
        h.assert_eq[PgDate](PgDate(8765), v)
...
class \nodoc\ iso
  _TestCompositeEncoderTimeRoundtrip
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    let registry =
      CodecRegistry.with_composite_type(
        16400,
        recover val
          [ as (String, U32): ("v", 1083)]
        end)?
    let us = MakePgTimeMicroseconds(43200000000)
    let t =
      match us
      | let valid: PgTimeMicroseconds =>
        PgTime(valid)
      else
        h.fail("Invalid time")
        return
...
class \nodoc\ iso
  _TestCompositeEncoderTimestampRoundtrip
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    | let d: PgComposite =>
      match d(0)?
      | let v: PgTimestamp =>
        h.assert_eq[PgTimestamp](
          PgTimestamp(694224000000000), v)
...
class \nodoc\ iso
  _TestCompositeEncoderIntervalRoundtrip
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    | let d: PgComposite =>
      match d(0)?
      | let v: PgInterval =>
        h.assert_eq[PgInterval](
          PgInterval(3600000000, 5, 2), v)
...
class \nodoc\ iso
  _TestCompositeEncoderUUIDRoundtrip
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestCompositeEncoderJSONBRoundtrip
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestCompositeEncoderOidRoundtrip
  is UnitTest
  fun name(): String =>
...
class \nodoc\ iso
  _TestCompositeEncoderNumericRoundtrip
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    | let d: PgComposite =>
      match d(0)?
      | let v: String =>
        h.assert_eq[String]("123456.789", v)
...
// _ParamEncoder tests
// ============================================================
class \nodoc\ iso _TestParamEncoderComposite
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    let oids =
      _ParamEncoder.oids_for(params, CodecRegistry)
...
// ============================================================
class \nodoc\ iso
  _TestFrontendMessageBindWithComposite
  is UnitTest
  fun name(): String =>
...
// ============================================================
class \nodoc\ iso
  _TestArrayWithCompositeElements
  is UnitTest
  fun name(): String =>
  ...
  fun apply(h: TestHelper) ? =>
    ...
    let arr =
      PgArray(
        16400,
        recover val
          [ as (FieldData | None): c1; c2]
        end)

    // Encode then decode roundtrip
    let encoded = _ArrayEncoder(arr, registry)?
    let decoded =
      registry.decode(16401, 1, encoded)?
    match decoded
    | let a: PgArray =>
...
class \nodoc\ iso _TestCompositeWithArrayField
  is UnitTest
  fun name(): String =>
...
// _FieldDataEq tests
// ============================================================
class \nodoc\ iso _TestFieldDataEqComposite
  is UnitTest
  fun name(): String =>
...
// ============================================================
class \nodoc\ iso
  _TestCompositeUnregisteredFallback
  is UnitTest
  fun name(): String =>
    "Codec/Composite/UnregisteredFallback"

  fun apply(h: TestHelper) ? =>
    // Binary format: unregistered composite OID
    // falls back to RawBytes
    let fields:
      Array[(U32, (Array[U8] val | None))] val =
        recover val
          [ as (U32, (Array[U8] val | None)):
    ...
    let text_data: Array[U8] val =
      "(hello,42)".array()
    let text_result =
      registry.decode(99999, 0, text_data)?
    match text_result
    | let s: String =>
      None // expected
    else
...
// ============================================================
class \nodoc\ iso
  _TestIntegrationCompositeSelectBinary
  is UnitTest
  """
  CREATE TYPE, SELECT composite literal via
  PreparedQuery (binary decode). Uses a single session.
  The composite OID is unknown to the default
  CodecRegistry, so the binary result falls back to
  RawBytes. This test verifies the fallback and the
  type creation flow.
  """
  fun name(): String =>
  ...
  fun apply(h: TestHelper) =>
    let info =
      _ConnectionTestConfiguration(h.env.vars)
...
class \nodoc\ iso
  _TestIntegrationCompositeSelectText
  is UnitTest
  """
  SELECT composite literal via SimpleQuery
  (text decode).
  """
  fun name(): String =>
...
class \nodoc\ iso
  _TestIntegrationCompositeRoundtrip
  is UnitTest
  """
  Two-phase: discover composite OID, then reconnect
  with registered CodecRegistry, INSERT PgComposite,
  SELECT back and verify decode.
  """
  fun name(): String =>
...
// Single-session client: create type, SELECT, verify
// result type, drop type. Since the session's
// CodecRegistry doesn't know the composite OID,
// PreparedQuery returns RawBytes and SimpleQuery
// returns String. Both are valid fallback behaviors.
...
actor \nodoc\ _CompositeSelectClient is
  ...
  =>
    session.execute(
      SimpleQuery(
        "DROP TYPE IF EXISTS _test_address_sel"),
      this)
  ...
  =>
    ...
    | 3 =>
      // Verify we got a result (fallback type since
      // OID not registered)
      match result
      | let rs: ResultSet =>
        try
          ...
          if _use_prepared then
            // Binary format, unregistered OID
            match field.value
  ...
  be pg_query_failed(
    ...
    query: Query,
    failure:
      (ErrorResponseMessage | ClientQueryError))
  =>
    match failure
...
// Two-phase roundtrip client: phase 1 creates type
// and discovers OID, phase 2 reconnects with
// registered CodecRegistry, inserts, selects back.
    ...
    | 5 =>
      ...
      | let rs: ResultSet =>
        try
          ...
          | let s: String =>
            _composite_oid = s.u32()?
          end
        end
      end
      if _composite_oid == 0 then
        _h.fail(
          "Failed to discover composite OID")
        _h.complete(false)
        session.close()
        return
      end
      ...
      let registry =
        try
          CodecRegistry.with_composite_type(
            _composite_oid, descriptors)?
        else
          _h.fail(
            "Failed to register composite type")
          _h.complete(false)
          session.close()
          return
        end
        ...
        try
          ...
          | let c: PgComposite =>
            ...
            | let s: String =>
              ...
              _h.fail(
                "Expected String for city")
            end
            match c.field("zip")?
            | let v: I32 =>
              _h.assert_eq[I32](97201, v)
            else
              _h.fail("Expected I32 for zip")
            end
          else
